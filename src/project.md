...
The present subproject will address these challenges in three successive research phases. The first phase will initiate collaborations with multiple Swedish and international stakeholders (e.g. MSB, social media platforms, EU) in order to collect and analyze digital content that is produced, spread, and amplified through IOs. Once a repository of disinformation content is built, the second phase conducts experiments that measure the causal effects of this content on audiences' attitudes (polarization), memory (EEG), interaction behavior (website clicks, traffic), and visual attention (eye-tracking). In the third phase, we build on the project's findings to propose technological and regulatory interventions that moderate, inoculate, or even reverse the polarizing effects of IOs and disinformation campaigns.  
